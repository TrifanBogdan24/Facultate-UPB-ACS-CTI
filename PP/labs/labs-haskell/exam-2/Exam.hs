...
get_level _ Empty = []
get_level 0 (Node val left right) = [val]
get_level n (Node val left right) = (get_level (n - 1) left) ++ (get_level (n - 1) right)
...
f :: [Int]
f = 1 : 1 : 1 : 1 : zipWith (+) f (drop 3 f)


f1 :: [Int]
f1 =  let fs = 1 : 1 : 1 : 1 : zipWith (+) fs (drop 3 fs) in fs


f2 :: [Int]
f2 = fs
  where
    fs = 1 : 1 : 1 : 1 : zipWith (+) fs (drop 3 fs)
...
{--
 ...
 - [1 2 3] 4 [5 6 7] -> shift R -> [4 1 2 3] 5 [6 7]
 -}

data Direction = L | R

shift ::  Direction -> Zipper a -> Zipper a
shift L (Zipper (x1:xs1) el l2) = Zipper xs1 x1 (el : l2)
shift L (Zipper [] el l2) = Zipper [] el l2
shift R (Zipper l1 el (x2:xs2)) = Zipper (el : l1) x2 xs2
shift R (Zipper l1 el []) = Zipper l1 el []
