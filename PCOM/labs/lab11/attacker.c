...
void handle_encrypted_message(int source, char *data, size_t size) {
    ...
    uint8_t *plaintext = decrypt((uint8_t *)data, &msg_size, key);
    hex_dump(plaintext, msg_size);
    free(plaintext);
}
...
void handle_dh_message(int source, char *data, size_t size) {
    ...
    case SRC_ALICE:
        if (!exchange_done) {
            ...
            memcpy(&share, data, sizeof(share));
            // Generate own secret
            secret = rand();
            // Calculate shared value
            uint32_t shared = modular_exponentiation(share, secret, P);
            // Overwrite Alice's message with our shared value
            memcpy(data, &shared, sizeof(shared));
            // Calculate key for Alice
            key_alice = derive_key(shared);
        } else {
            // Decrypt Alice's message
            uint32_t msg_size = size;
            uint8_t *plaintext = decrypt((uint8_t *)data, &msg_size, key_alice);
            puts("ALICE wrote to BOB: ");
            hex_dump(plaintext, msg_size);
            free(plaintext);
            // Encrypt back for Bob
            uint8_t *ciphertext = encrypt((uint8_t *)data, &msg_size, key_bob);
            memcpy(data, ciphertext, msg_size);
            free(ciphertext);
        }
        break;
    case SRC_BOB:
        if (!exchange_done) {
            ...
            // Calculate shared value
            uint32_t shared = modular_exponentiation(share, secret, P);
            // Overwrite Bob's message with our shared value
            memcpy(data, &shared, sizeof(shared));
            // Calculate key for Bob
            key_bob = derive_key(shared);
            exchange_done = 1;
        } else {
            // Decrypt Bob's message
            uint32_t msg_size = size;
            uint8_t *plaintext = decrypt((uint8_t *)data, &msg_size, key_bob);
            puts("BOB wrote to ALICE: ");
            hex_dump(plaintext, msg_size);
            free(plaintext);
            // Encrypt back for Alice
            uint8_t *ciphertext = encrypt((uint8_t *)data, &msg_size, key_alice);
            memcpy(data, ciphertext, msg_size);
            free(ciphertext);
        }
        break;
